...
def create_planner(current_user: dict = Depends(get_current_user)):
    """Tạo planner cho tuần hiện tại dựa trên study_days và study_level."""
    ...
    try:
        ...
        for i in range(7):
            ...
            cur.execute(
                """
                INSERT INTO planner_day (planner_id, study_date, day_of_week, status)
            ...
            for task_type, title, desc, total in tasks_template:
                cur.execute(
                    """
                    INSERT INTO task
                        (planner_day_id, task_type, title, description,
                         total_required, progress_count, status)
                    VALUES (%s, %s, %s, %s, %s, 0, 'pending')
                    """,
                    (planner_day_id, task_type, title, desc, total),
                )
...
def get_current_planner(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        cur.execute(
            """
            SELECT id, study_date, day_of_week, status
            FROM planner_day WHERE planner_id = %s
        ...
        for day in days:
            ...
            cur.execute(
                ...
                SELECT id, task_type, title, description,
                       total_required, progress_count, status
                FROM task WHERE planner_day_id = %s ORDER BY id ASC
                """,
                (day_id,),
            )
            tasks = [
        ...
        "data": [
            {
                ...
                "week_start": str(p[1]),
                "week_end":   str(p[2]),
                "created_at": p[3].strftime("%Y-%m-%d %H:%M:%S"),
            }
            for p in planners
        ],
    }
...
def update_task_progress(
    ...
    try:
        ...
        cur.execute(
            "SELECT id, planner_day_id, total_required FROM task WHERE id = %s",
            (payload.task_id,),
        )
        task = cur.fetchone()
        if not task:
        ...
        task_id, planner_day_id, total_required = task

        status = "completed" if payload.progress_count >= total_required else "pending"

        cur.execute(
            "UPDATE task SET progress_count = %s, status = %s, updated_at = NOW() WHERE id = %s",
            (payload.progress_count, status, task_id),
        )

        # Kiểm tra còn task pending không → cập nhật planner_day
        cur.execute(
            "SELECT COUNT(*) FROM task WHERE planner_day_id = %s AND status != 'completed'",
            (planner_day_id,),
        )
        remaining           = cur.fetchone()[0]
        planner_day_status  = "completed" if remaining == 0 else "pending"

        cur.execute(
            "UPDATE planner_day SET status = %s, updated_at = NOW() WHERE id = %s",
            (planner_day_status, planner_day_id),
        )
