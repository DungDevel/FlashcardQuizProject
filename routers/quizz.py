"""
routers/quizz.py — Quiz (AI generate + SRS + Session Retry Queue + progress)
...
Endpoints:
  ...
  POST   /quiz/session/start         — Khởi tạo session (dùng với retry queue)
  POST   /quiz/session/answer        — Nộp đáp án trong session (trả về should_retry)
...
BANNED_WORDS = [
    "sex", "porn", "rape", "terrorist", "fuck", "shit",
    "bitch", "nigger", "whore", "suicide", "weapon",
]
...
def calculate_next_review(is_correct: bool, ease: float, interval: int, review_count: int):
    """Tính ease factor, interval và ngày ôn tiếp theo theo thuật toán SM-2."""
    now = datetime.now()
    if not is_correct:
...
async def generate_quiz(
    ...
    instruction_map = {
        "multiple": (
            ...
            "    Do NOT repeat or copy the full answer text. "
            "    The letter must correspond to the correct option.\n"
...
def get_quizzes_by_deck(
    ...
    """Lấy toàn bộ quiz theo deck (kèm đáp án — dùng cho màn hình quản lý)."""
    conn = get_connection()
    try:
        cur = conn.cursor()
        if quiz_type:
            cur.execute(
                """
                SELECT id, question, question_type, options, correct_answer, context
                FROM quiz
                WHERE deck_id = %s AND question_type = %s
                ORDER BY id ASC
                """,
                (deck_id, quiz_type),
            )
            ...
            cur.execute(
                """
                SELECT id, question, question_type, options, correct_answer, context
                FROM quiz WHERE deck_id = %s ORDER BY id ASC
                """,
                (deck_id,),
            )
        rows = cur.fetchall()
...
def get_practice_quiz(
    ...
    """
    Lấy danh sách quiz để luyện tập (không kèm đáp án).
    Ưu tiên: chưa làm > đã sai > đã đúng.
    """
        ...
        cur.execute(
            ...
            ORDER BY
                CASE WHEN p.id IS NULL      THEN 0
                     WHEN p.is_correct = FALSE THEN 1
                     ELSE 2
                END,
                RANDOM()
            LIMIT %s
...
# ─────────────────────────────────────────
# SESSION-BASED QUIZ — Retry Queue
# ─────────────────────────────────────────

@router.post("/quiz/session/start")
def start_quiz_session(
    deck_id:      int  = Form(...),
    quiz_type:    str  = Form(None),
    limit:        int  = Form(20),
    current_user: dict = Depends(get_current_user),
):
    """
    Khởi tạo session quiz.
    Trả về ordered list câu hỏi (không kèm đáp án).
    Frontend dùng list này để xây dựng retry queue trong memory.

    Thứ tự ưu tiên:
      1. Chưa từng làm
      2. Đã làm sai
      3. Đã làm đúng (ôn lại)
    """
    user_id = current_user["id"]
    conn    = get_connection()
    try:
    ...
    return {
        ...
        "total":     len(questions),
        "questions": questions,
        # Frontend nhận list này, tự quản lý retry queue trong React state.
        # Khi is_correct=False, frontend splice câu hỏi vào index+2 trong queue.
    }


@router.post("/quiz/session/answer")
def answer_quiz_session(
    quiz_id:      int  = Form(...),
    user_answer:  str  = Form(...),
    current_user: dict = Depends(get_current_user),
):
    """
    Nộp đáp án trong session.

    Response:
      - is_correct    : bool
      - should_retry  : bool  ← KEY FLAG. True khi sai → frontend chèn câu này vào queue[current+2]
      - correct_answer: str | null  (chỉ trả về khi sai, để hiển thị feedback)
      - explanation   : str | null  (context hint, chỉ khi sai)
      - srs           : object (ease_factor, interval_days, next_review, review_count)
      - task_updated  : bool
    """
    user_id = current_user["id"]
    conn    = get_connection()
    try:
        ...
        # Lấy thông tin quiz
        cur.execute(
            "SELECT deck_id, question_type, correct_answer, context, question FROM quiz WHERE id = %s",
            (quiz_id,),
        )
        quiz = cur.fetchone()
        if not quiz:
        ...
        return {
            ...
            # Frontend kiểm tra flag này để quyết định có splice câu vào queue hay không
            "should_retry":   not is_correct,
            # Chỉ trả về đáp án + giải thích khi sai (tránh spoil khi đúng)
            "correct_answer": correct_answer if not is_correct else None,
            "explanation":    (context or "Không có giải thích.") if not is_correct else None,
...
def submit_quiz(
    ...
    """Nộp đáp án đơn giản (không SRS, không session)."""
    user_id = current_user["id"]
    conn    = get_connection()
    try:
        cur = conn.cursor()
        cur.execute(
            "SELECT deck_id, question_type, correct_answer FROM quiz WHERE id = %s",
            (quiz_id,),
        )
        quiz = cur.fetchone()
        if not quiz:
        ...
        cur.execute(
            "SELECT id, status FROM user_quiz_progress WHERE user_id = %s AND quiz_id = %s",
            (user_id, quiz_id),
        )
        existing = cur.fetchone()
        ...
        if existing:
            ...
            cur.execute(
                """
                INSERT INTO user_quiz_progress
                    (user_id, quiz_id, deck_id, quiz_type, user_answer, is_correct, status, attempt_count)
...
def submit_quiz_srs(
    ...
    Dùng cho flow luyện tập riêng lẻ (không phải session retry queue).
    Với session retry queue hãy dùng POST /quiz/session/answer.
    """
    user_id = current_user["id"]
    conn    = get_connection()
    try:
        ...
        if existing:
            ...
            cur.execute(
                ...
                SET user_answer      = %s,
                    ...
                    last_review_date = NOW(),
                    attempt_count    = attempt_count + 1,
                    status           = CASE WHEN %s THEN 'completed' ELSE 'reviewing' END,
                    updated_at       = NOW()
                WHERE id = %s
                """,
                (user_answer, is_correct, new_ease, new_interval,
                 next_review, new_count, is_correct, prog_id),
            )
            was_completed = old_status == "completed"
        else:
            new_ease = INITIAL_EASE_FACTOR
...
def get_due_review(
    ...
    """Lấy danh sách quiz cần ôn lại hôm nay (next_review_date <= NOW)."""
    user_id = current_user["id"]
    conn    = get_connection()
    try:
        cur = conn.cursor()
    ...
    data = [
        {
            ...
            "context":  r[4],
            "srs": {
                "ease_factor":   float(r[5] or 2.5),
                "interval_days": r[6],
                "next_review":   r[7].strftime("%Y-%m-%d") if r[7] else None,
                "review_count":  r[8],
            },
        }
        for r in rows
...
def get_quiz_progress(
    ...
    """Tiến độ làm quiz của user theo deck."""
    user_id = current_user["id"]
    conn    = get_connection()
    try:
        cur        = conn.cursor()
...
def get_srs_stats(
    ...
    """Thống kê SRS tổng quan cho deck."""
    user_id = current_user["id"]
    conn    = get_connection()
    try:
        cur = conn.cursor()
...
def delete_quiz(
    ...
    """Xoá quiz (chỉ owner của deck mới được xoá)."""
    user_id = current_user["id"]
    conn    = get_connection()
    try:
        cur = conn.cursor()
        cur.execute(
            "SELECT d.user_id FROM quiz q JOIN deck d ON q.deck_id = d.id WHERE q.id = %s",
            (quiz_id,),
        )
        result = cur.fetchone()
        if not result:
